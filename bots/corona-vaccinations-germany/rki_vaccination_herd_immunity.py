...
def updateChart(id, title="", subtitle="", notes="", data=pd.DataFrame()):
    # read qConfig file
        ...
        for environment in item.get('environments'):
            if environment.get('id') == id:
                ...
                if data.size > 0:
                    ...
                    item.get('item').update({'data': data.applymap(
                        str).reset_index(drop=False).T.reset_index().T.apply(list, axis=1).to_list()})
...
if __name__ == '__main__':
    try:
        ...
        updateChart(id='dd4b1de66b3907bb65164669b0cca8dd', data=df2, title=title_chart,
                    subtitle='Anteil der geimpften Bevölkerung in Prozent', notes=notes_chart)
